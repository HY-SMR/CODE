...
dataset=pd.read_csv("fc12309.csv",encoding='gb2312')
...
num_samples = 2300  # 可以设定从数据中取出多少个点用于本次网络的训练与测试。
...
n_train_number = int(num_samples * 0.55)
# 计算训练集的大小。
# 设置80%作为训练集
# int(...) 确保得到的训练集大小是一个整数。
...
history = model.fit(vp_train, vt_train, batch_size=72, epochs=30, validation_split=0.25, verbose=2)
...
plt.show()
# 显示图形。
result_df=pd.DataFrame({'真实值':Ytest.flatten(),'预测值':predicted_data.flatten()})
result_df.to_csv('真实值与预测值GRU.csv',index=False,encoding='utf-8')
'''
欢迎关注《淘个代码》公众号。
如代码有问题，请公众号后台留言问题！不要问在吗在吗。
直接截图留言问题！
